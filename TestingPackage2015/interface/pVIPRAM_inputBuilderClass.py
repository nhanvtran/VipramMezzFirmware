...
#        else: binaryVal[0] = "1";
    ...
    # initializeRunPhase_Mod
    def initializeRunPhase_Mod(self, logic=[1,1,1,1],initialRow = 1,iValU=[0,0,0,0]):
        #print "initializing run phase..."
...
#            self.tree.Fill();
    ...
    # loadRowChecker
    def doRowChecker( self, row, col = 1, delay=0 ):
        ...
        self.ColAdr[0] = col;
        for i in range(32): 
            self.DataOut[i] = self.iDataOut[row][i];
            #print self.iDataOut[row];

        self.counter[0] = self.cycleCtr;
        self.cycleCtr += 1;
        for i in range(delay): self.tree.Fill();
        self.CheckData[0] = 1;                    
        self.tree.Fill()
    ...
    # readOutMode_M
    def readOutMode_withDelay(self, freq=100):
        for i in range(128): 
			#for k in range (int(freq/2+1)):
            #for k in range ( freq ):   
            self.doRowChecker(i,1,freq);
