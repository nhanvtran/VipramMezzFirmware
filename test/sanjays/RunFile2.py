...
if __name__ == '__main__':
    ...
    pattern1 = GenerateInputsRun("tmp1");
...
if __name__ == '__main__':
    ...
    expectedFile = open("Expected CERN Outputs1.txt", "r+")
    expectedLines = expectedFile.readlines()
    inputFile = open(sys.argv[1], "r+")
    inputLines = inputFile.readlines()
    count = 0
    #for i in range(runModeLine1,len(list1)):
    #for count in range(0, len(expectedLines)):
	...
	#checkData = 1
	    ...
	    expectedInfo = expectedLines[count].split(" ")
	    rowE = int(expectedInfo[0])
	    colE = 0
	    comp1 = ""
	    for j in range(((rowE+1)*32), (rowE*32), -1):
		data = inputLines[j].split(" - ")
		flag = 0
	        for k in range(0, 4):
		    if (int(data[k]) != int(expectedInfo[k+1])):
		        flag = 1
		        break
	        comp1 += str(1-flag)
	    count += 1
...
#            outputPattern[row] = outputData
		...
		print "Comparison: " + str(comp1) + ", " + str(comp2)
                mismatchCtr += 1;
	    else:
	    	matchCtr += 1;
	#for m in range(i+1, len(list2)):
	    #print list2[m]
	    #if (list2[m][39] == '1'):
		#i = m+(2*shift)
		#break
